'use client';

import * as React from 'react';
import type { AnalyzeDemandTrendsOutput } from '@/ai/flows/analyze-demand-trends';
import type { SummarizeHighDemandOutput } from '@/ai/flows/summarize-high-demand';
import type { Route, PriceTrend, KpiData } from '@/lib/types';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { KpiCard } from './kpi-card';
import { PriceTrendsChart } from './price-trends-chart';
import { PopularRoutesTable } from './popular-routes-table';
import { Plane, Lightbulb, TrendingUp, Users, DollarSign } from 'lucide-react';

interface AirDemandDashboardProps {
  analysis: AnalyzeDemandTrendsOutput;
  summary: SummarizeHighDemandOutput;
  routesData: Route[];
  priceTrendsData: PriceTrend[];
}

export function AirDemandDashboard({ analysis, summary, routesData, priceTrendsData }: AirDemandDashboardProps) {
  const [currentYear, setCurrentYear] = React.useState<number | string>(new Date().getFullYear());

  React.useEffect(() => {
    setCurrentYear(new Date().getFullYear());
  }, []);

  const kpiData: KpiData[] = [
    {
      title: 'Popular Routes',
      value: `${routesData.length}`,
      description: 'Total monitored routes',
      icon: Plane,
      colorClassName: 'text-sky-500',
    },
    {
      title: 'Demand Trend',
      value: 'Rising',
      description: 'Overall market demand',
      icon: TrendingUp,
      colorClassName: 'text-green-500',
    },
    {
      title: 'Highest Searches',
      value: routesData.reduce((max, route) => (route.searches > max ? route.searches : max), 0).toLocaleString(),
      description: 'Peak search count on a route',
      icon: Users,
      colorClassName: 'text-amber-500',
    },
    {
      title: 'Avg. Price',
      value: `$${(
        priceTrendsData.reduce((acc, trend) => acc + trend['Average Price'], 0) / priceTrendsData.length
      ).toFixed(2)}`,
      description: 'Across all routes',
      icon: DollarSign,
      colorClassName: 'text-blue-500',
    },
  ];

  return (
    <div className="flex flex-col min-h-screen font-body">
      <header className="sticky top-0 z-40 w-full border-b bg-card">
        <div className="container flex h-16 items-center space-x-4">
          <Plane className="h-8 w-8 text-primary" />
          <h1 className="text-2xl font-bold tracking-tight font-headline">AirDemand Insights</h1>
        </div>
      </header>
      <main className="flex-1 p-4 md:p-8 container">
        <div className="grid gap-6 md:gap-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {kpiData.map((kpi) => (
              <KpiCard key={kpi.title} {...kpi} />
            ))}
          </div>

          <div className="grid lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-1">
              <CardHeader>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-accent/20 rounded-full">
                    <Lightbulb className="h-6 w-6 text-accent-foreground" />
                  </div>
                  <CardTitle className="font-headline">AI-Powered Summary</CardTitle>
                </div>
                <CardDescription>{summary.summary}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4 text-sm">
                  <div>
                    <h4 className="font-semibold mb-1">Demand Trends</h4>
                    <p className="text-muted-foreground">{analysis.demandTrends}</p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-1">High-Demand Periods</h4>
                    <p className="text-muted-foreground">{analysis.highDemandPeriods}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <PriceTrendsChart data={priceTrendsData} />
          </div>

          <PopularRoutesTable data={routesData} />
        </div>
      </main>
      <footer className="py-4 text-center text-sm text-muted-foreground">
        © {currentYear} AirDemand Insights. All rights reserved.
      </footer>
    </div>
  );
}
